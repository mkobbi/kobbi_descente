...
public class Descente extends Observable implements Runnable  {
	...
	private void directionASuivreSiInterieur() {
		Couple grad = problemeTraite.gradientf(pointCourant);
		if(grad.norme()<=this.seuil){
			this.finie=true;
		} else{
			this.directionASuivre = grad.mult(-1);
		}
	}
	...
	private void directionASuivreSiBord(Contrainte c) {
		...
		if(grad.norme()<=this.seuil){
			...
			Couple u = c.getBordUnitaire();
			Couple n = c.getGradient();
			Couple w = n.mult(-1.0/n.norme());
			if (grad.estPerpendiculaire(n)==true){
				this.directionASuivre=grad;
			} else {
				double a = grad.produitScalaire(u);
				double b = grad.produitScalaire(w);
				if (a < 0) {
					a = -a;
				}
				if (b < 0){
				...
				this.directionASuivre= u.mult(a).ajoute(w.mult(b));
			}
	...
	* Si la descente n'est pas finie, met la prochaine direction dans l'attribut this.directionASuivre  
	...
	private void directionASuivreSiCoin(Contrainte[] coin) {
		...
		if(grad.norme()<=this.seuil){
			...
			Couple n1 = coin[0].getGradient().mult(-1.0/coin[0].getGradient().norme());
			Couple u2 = coin[1].getBordUnitaire();
			Couple n2 = coin[1].getGradient().mult(-1.0/coin[1].getGradient().norme());
			if (n1.produitScalaire(grad)<=0 && n2.produitScalaire(grad) <= 0){
				this.directionASuivre = grad.mult(-1.0);
			} else {
				double p1 = u1.produitScalaire(grad), p2 = u2.produitScalaire(grad);
				if (p1 >= 0 && p2 >= 0 ){
					this.finie=true;
				} else {
					if (p1 <= p2){
